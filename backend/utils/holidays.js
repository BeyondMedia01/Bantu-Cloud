...
async function autoSeedHolidays() {
  try {
    const year = new Date().getFullYear();
    const nextYear = year + 1;
    
    for (const targetYear of [year, nextYear]) {
      const existing = await prisma.publicHoliday.findFirst({
        where: { year: targetYear, country: 'ZW' },
      });
      ...
      if (created > 0) {
        console.log(`Auto-seeded ${created} public holidays for ${targetYear}.`);
      }
    }
  } catch (error) {
    console.error('Failed to auto-seed holidays:', error);
  }
}

module.exports = { buildZimbabweHolidays, autoSeedHolidays };
