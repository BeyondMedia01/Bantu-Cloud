  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form); }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
        ...
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">Start Time *</label>
          <input type="time" value={form.startTime} onChange={set('startTime')} required
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">End Time *</label>
          <input type="time" value={form.endTime} onChange={set('endTime')} required
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">Break (minutes)</label>
          <input type="number" min="0" max="480" value={form.breakMinutes} onChange={set('breakMinutes')}
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">Normal Hours / Day</label>
          <input type="number" min="1" max="24" step="0.5" value={form.normalHours} onChange={set('normalHours')}
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">OT 1.0 Threshold (hrs)</label>
          <input type="number" min="0" max="24" step="0.5" value={form.ot0Threshold} onChange={set('ot0Threshold')}
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">OT 1.0 Multiplier</label>
          <input type="number" min="0" max="10" step="0.1" value={form.ot0Multiplier} onChange={set('ot0Multiplier')}
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">OT 1.5 Threshold (hrs)</label>
          <input type="number" min="0" max="24" step="0.5" value={form.ot1Threshold} onChange={set('ot1Threshold')}
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">OT 1.5 Multiplier</label>
          <input type="number" min="0" max="10" step="0.1" value={form.ot1Multiplier} onChange={set('ot1Multiplier')}
            className="w-full px-3 py-2 border border-border rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-green/30 focus:border-accent-green" />
        </div>
        <div>
          <label className="block text-xs font-bold text-foreground/80 mb-1.5">OT 2.0 Multiplier</label>
        ...
        <div className="flex items-center gap-3 pt-5">
          ...
          <label htmlFor="overnight" className="text-sm font-medium text-foreground/80">Overnight shift (crosses midnight)</label>
        </div>
      </div>
                ...
                <div className={`bg-primary border border-border rounded-2xl p-5 shadow-sm ${!s.isActive ? 'opacity-60' : ''}`}>
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      {s.code && <p className="text-xs text-muted-foreground font-semibold mt-0.5">{s.code}</p>}
                    </div>
                    {!s.isActive && (
                  ...
                  <div className="flex items-center gap-2 text-sm text-foreground/80 font-medium mb-1">
                  ...
                  <div className="text-xs text-muted-foreground font-medium space-y-0.5 mb-3">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium border-t border-border pt-3">
                    <Users size={12} />
