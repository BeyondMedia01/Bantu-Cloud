    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 font-medium text-sm">Raw biometric logs and processed daily records</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setShowManual(true)}
            className="flex items-center gap-1.5 px-4 py-2 border border-border rounded-full font-bold text-sm text-slate-600 hover:bg-slate-50">
        ...
        {(['records', 'logs'] as Tab[]).map((t) => (
          <button key={t} onClick={() => { setTab(t); setPage(1); }}
            className={`px-5 py-1.5 rounded-lg font-bold text-sm transition-colors ${
              tab === t ? 'bg-white shadow-sm text-navy' : 'text-slate-500 hover:text-navy'
            }`}>
                ...
                {records.length === 0 ? (
                  ...
                  <tr key={r.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[10px] text-slate-400">{r.employee?.employeeCode}</div>
                    </td>
                    <td className="px-4 py-3 text-sm font-medium text-slate-600">{fmtDate(r.date)}</td>
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-black uppercase tracking-wide px-2 py-0.5 rounded-full ${STATUS_COLORS[r.status] || 'bg-slate-100 text-slate-500'}`}>
                        {r.status?.replace('_', ' ')}
                      </span>
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-slate-600">{fmtTime(r.clockIn)}</td>
                    ...
                    <td className="px-4 py-3">
                      <button className="text-slate-400 hover:text-navy p-1 rounded-lg hover:bg-slate-100">
                        <Edit2 size={13} />
                      </button>
                    </td>
                  </tr>
                ...
                {logs.length === 0 ? (
                  ...
                  <tr key={l.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[10px] text-slate-400">{l.employee?.employeeCode}</div>
                    </td>
                    <td className="px-4 py-3 text-sm font-medium text-slate-600">
                      <div>{fmtDate(l.punchTime)}</div>
                      <div className="text-xs text-slate-400">{fmtTime(l.punchTime)}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-black uppercase tracking-wide px-2 py-0.5 rounded-full ${PUNCH_COLORS[l.punchType] || 'bg-slate-100 text-slate-500'}`}>
                        {l.punchType?.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-xs font-mono text-slate-500">{l.deviceUserId}</td>
                    <td className="px-4 py-3 text-xs font-semibold text-slate-500">{l.source}</td>
                    <td className="px-4 py-3">
                      {l.processed
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                className="px-3 py-1.5 text-xs font-bold border border-border rounded-lg disabled:opacity-40 hover:bg-slate-50">Prev</button>
              <span className="px-3 py-1.5 text-xs font-bold">Page {page} / {totalPages}</span>
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                className="px-3 py-1.5 text-xs font-bold border border-border rounded-lg disabled:opacity-40 hover:bg-slate-50">Next</button>
            </div>
          </div>
        )}
      </div>
