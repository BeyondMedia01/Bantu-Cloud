    ...
    <div className="flex flex-col gap-8">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground font-medium">Manage multiple business entities on one platform.</p>
        </div>
        <button
        ...
        <div className="bg-primary rounded-2xl border border-border shadow-sm p-8 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Company Legal Name</label>
              <input type="text" required className={inputCls} placeholder="Zimbabwe Tech Ltd"
                value={newName} onChange={e => setNewName(e.target.value)} />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">BP Number (ZIMRA)</label>
              <input type="text" className={inputCls} placeholder="200XXXXXX"
                value={newBp} onChange={e => setNewBp(e.target.value)} />
            </div>
            <div className="md:col-span-2 flex flex-col gap-2">
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Primary Address</label>
            ...
            {editingId === company.id ? (
              ...
              <div className="p-6 flex flex-col h-full group">
                <div className="flex items-start gap-4 flex-1">
                  <div className="w-12 h-12 bg-muted rounded-xl flex items-center justify-center text-accent-green shrink-0
