    ...
    <div className="flex flex-col gap-8 relative">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground font-medium">Monthly statutory pension tracking and compliance ledger.</p>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={() => setShowSettings(true)}
            className="bg-background border border-border text-navy px-4 py-2 rounded-full font-bold hover:bg-muted transition-colors flex items-center gap-1.5"
          >
            <Settings size={20} /> Configure NSSA
          </button>
          <button className="bg-muted text-navy px-4 py-2 rounded-full font-bold hover:bg-muted transition-colors flex items-center gap-1.5">
        ...
        <div className="p-4 px-6 border-b border-border bg-muted/30 flex items-center justify-between">
            <div className="relative flex-1 max-w-md">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-background border border-border rounded-xl text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30"
                value={search}
                ...
                <tr key={c.id} className="hover:bg-muted/30 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{new Date(c.payPeriod).toLocaleDateString(undefined, { month: 'short', year: 'numeric' })}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] text-muted-foreground font-bold uppercase mb-0.5">ER ({settings.employerRate}%)</p>
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      {c.submittedToNSSA ? <CheckCircle size={14} /> : <div className="w-3.5 h-3.5 border-2 border-border rounded-full" />}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-navy/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-card rounded-[32px] w-full max-w-lg shadow-2xl border border-white/20 overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-8 pb-0 flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-muted-foreground font-medium">Global statutory rates and contribution ceilings.</p>
              </div>
              <button onClick={() => setShowSettings(false)} className="p-2 hover:bg-muted rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveSettings} className="p-8 flex flex-col gap-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-2 col-span-2">
                  <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Insurable Ceiling (USD)</label>
                  <input 
                    type="number" step="0.01" required
                    className="w-full px-4 py-3 bg-muted border border-border rounded-xl font-bold text-accent-green text-lg outline-none focus:ring-2 focus:ring-accent-green/10 focus:border-accent-green transition-all"
                    value={settings.ceilingUSD}
                    onChange={e => setSettings({...settings, ceilingUSD: parseFloat(e.target.value)})}
                  />
                </div>
                <div className="flex flex-col gap-2 col-span-2">
                  <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">WCIF Rate (%)</label>
                  <input 
                    type="number" step="0.01" required
                    className="w-full px-4 py-3 bg-muted border border-border rounded-xl font-bold text-navy outline-none focus:ring-2 focus:ring-accent-green/10 focus:border-accent-green transition-all"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button 
                  ...
                  className="px-6 py-4 bg-muted rounded-2xl font-bold text-muted-foreground hover:bg-muted transition-colors"
                >
                  Cancel
                </button>
              </div>
