          ...
          {STEPS.map((label, i) => {
            ...
            return (
              <React.Fragment key={n}>
                ...
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-1 mt-3.5 ${step > n ? 'bg-emerald-400' : 'bg-slate-200'}`} />
                )}
              </React.Fragment>
            );
          })}
                  ...
                  {TX_TYPES.map((t) => (
                    <button
                      ...
                      className={`py-2.5 rounded-xl text-sm font-bold border transition-colors ${
                        form.type === t ? `${TYPE_COLOR[t]} border-current` : 'border-border text-slate-400 hover:bg-slate-50'
                      }`}
                    >
                      {t}
                    </button>
          ...
          {step === 3 && (
            <div className="flex flex-col gap-4">
              ...
              {form.calculationType === 'fixed' && (
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Default Amount</label>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Can be overridden per employee in Payroll Inputs</p>
                </div>
              )}

              {form.calculationType === 'percentage' && (
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Percentage of Basic (%)</label>
              ...
              {form.calculationType === 'formula' && (
                <div>
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Formula Expression *</label>
                  <textarea
                    value={form.formula}
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Variables: <code className="bg-slate-100 px-1 rounded">baseSalary</code>{' '}
                    <code className="bg-slate-100 px-1 rounded">overtime</code>{' '}
                    <code className="bg-slate-100 px-1 rounded">hoursWorked</code>
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="p-4 bg-navy/5 border border-navy/10 rounded-xl">
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
                  ...
                  {!form.taxable && <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full font-bold">Non-Taxable</span>}
                </div>
              </div>
              ...
              {editData ? (
                <div>
                  ...
                  {showRuleForm && (
                    <form onSubmit={handleAddRule} className="mb-3 p-4 bg-slate-50 border border-border rounded-xl flex flex-col gap-3">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        {ruleForm.conditionType !== 'always' && (
                          <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Condition Value</label>
                            <input value={ruleForm.conditionValue} onChange={(e) => setRuleForm((p) => ({ ...p, conditionValue: e.target.value }))}
                              placeholder="e.g. 700 or A" className="w-full px-3 py-2 bg-white border border-border rounded-lg text-xs font-medium focus:outline-none" />
                          </div>
                        )}
                        <div>
                          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Override Value / Rate</label>
                          <input type="number" step="0.01" value={ruleForm.valueOverride} onChange={(e) => setRuleForm((p) => ({ ...p, valueOverride: e.target.value }))}
                            placeholder="e.g. 1.5 or 700" className="w-full px-3 py-2 bg-white border border-border rounded-lg text-xs font-medium focus:outline-none" />
                        </div>
                        <div>
                          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Cap Amount</label>
                          <input type="number" step="0.01" value={ruleForm.capAmount} onChange={(e) => setRuleForm((p) => ({ ...p, capAmount: e.target.value }))}
                            placeholder="e.g. 700 (NSSA cap)" className="w-full px-3 py-2 bg-white border border-border rounded-lg text-xs font-medium focus:outline-none" />
                        </div>
                      </div>
                  ...
                  {rules.length > 0 ? (
                    <div className="flex flex-col gap-1.5">
                      {rules.map((rule: any) => (
                        <div key={rule.id} className="flex items-start justify-between gap-2 px-3 py-2.5 bg-slate-50 border border-border rounded-xl">
                          <div>
                          ...
                          <button onClick={() => handleDeleteRule(rule.id)} className="p-1 text-slate-300 hover:text-red-400 flex-shrink-0">
                            <Trash2 size={12} />
                          </button>
                        </div>
                      ))}
                    ...
                    <p className="text-xs text-slate-400 py-2">No rules — this code applies its default calculation unconditionally.</p>
                  )}
                </div>
              ) : (
            ...
            grouped[type].length > 0 && (
              <div key={type} className="bg-primary rounded-2xl border border-border shadow-sm overflow-hidden">
                <div className={`px-5 py-3 border-b border-border flex items-center gap-2 ${
                  ...
                  <span className="text-xs text-slate-400 font-medium">{grouped[type].length} code(s)</span>
                </div>
                    ...
                    {grouped[type].map((c: any) => (
                      <React.Fragment key={c.id}>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${c.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              {expanded === c.id ? <ChevronUp size={14} className="text-slate-400" /> : <ChevronDown size={14} className="text-slate-400" />}
                            </div>
                          </td>
                        </tr>
                        ...
                        {expanded === c.id && (
                          <tr>
                            <td colSpan={7} className="px-5 pb-4 bg-slate-50/60">
                              <div className="pt-3 grid grid-cols-2 gap-4 text-xs">
                                <div>
                                  <p className="font-bold text-slate-400 uppercase tracking-wider mb-1">Calculation</p>
                                  <p className="font-mono text-slate-700">{livePreview(c)}</p>
                                  {c.formula && <p className="font-mono text-slate-500 mt-1">{c.formula}</p>}
                                </div>
                                <div>
                                ...
                                {c.rules?.length > 0 && (
                                  <div className="col-span-2">
                                    <p className="font-bold text-slate-400 uppercase tracking-wider mb-1">Conditional Rules</p>
                                    <div className="flex flex-col gap-1">
                                      {c.rules.map((r: any) => (
                                        <div key={r.id} className="px-3 py-2 bg-white border border-border rounded-lg">
